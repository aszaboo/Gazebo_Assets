...
def export_sdf(prefix_path):
    ...
    for o in mesh_objects:
        ...
        diffuse_map = ""
        # Check if the object has a material and a node tree
        if o.active_material and o.active_material.node_tree:
            for item in o.active_material.node_tree.interface.items_tree:
                if item.item_type == 'SOCKET' and item.in_out == 'INPUT':
                    # Iterate over the inputs to find the Base Color
                    for node in o.active_material.node_tree.nodes:
                        if node.type == 'BSDF_PRINCIPLED':
                            base_color_input = node.inputs.get('Base Color')
                            if base_color_input:
                                # Assuming the link is not None, get the linked node's image name if it's an image texture node
                                if base_color_input.is_linked:
                                    link_node = base_color_input.links[0].from_node
    ...
    def add_attenuation_tags(light_tag, blender_light):
        ...
        color = ET.SubElement(light_tag, "color")
        color.text = f"{blender_light.color[0]} {blender_light.color[1]} {blender_light.color[2]}"
        
        # Add cutoff distance if custom distance is used, otherwise, add a default or calculated value
        cutoff = ET.SubElement(light_tag, "cutoff_distance")
        if blender_light.use_custom_distance:
            cutoff.text = str(blender_light.cutoff_distance)
        else:
            # Assuming a default or calculated value if not using custom distance
            cutoff.text = "25.0"  # Example value, adjust as needed
        
        # Add intensity (diffuse_factor can serve as an intensity multiplier)
        intensity = ET.SubElement(light_tag, "intensity")
        intensity.text = str(blender_light.diffuse_factor)

        # Optionally, add specular_factor if relevant to the export format
        specular = ET.SubElement(light_tag, "specular_factor")
        specular.text = str(blender_light.specular_factor)
        
        # If there are more specific properties you wish to export based on the light type, you can check and add them here
        # For example, volume intensity multiplier for volumetric lights
        if blender_light.type in {'POINT', 'SPOT', 'SUN', 'AREA'}:
            volume = ET.SubElement(light_tag, "volume_factor")
            volume.text = str(blender_light.volume_factor)
